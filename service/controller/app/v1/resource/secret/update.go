...
func (r *Resource) ApplyUpdateChange(ctx context.Context, obj, updateChange interface{}) error {
	...
	if secret.Name != "" {
		r.logger.LogCtx(ctx, "level", "debug", "message", fmt.Sprintf("ensuring update of secret %#q", secret.Name))
		...
		_, err = ctlCtx.K8sClient.CoreV1().Secrets(secret.Namespace).Update(secret)
		if err != nil {
			return microerror.Mask(err)
		}

		r.logger.LogCtx(ctx, "level", "debug", "message", fmt.Sprintf("ensured update of secret %#q", secret.Name))
	} else {
		r.logger.LogCtx(ctx, "level", "debug", "message", fmt.Sprintf("no need to update secret"))
	}
	return nil
}
