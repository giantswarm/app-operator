...
func (r *Resource) validateApp(ctx context.Context, cr v1alpha1.App) error {
	if key.AppConfigMapName(cr) != "" {
		...
		_, err := r.k8sClient.CoreV1().ConfigMaps(ns).Get(key.AppConfigMapName(cr), metav1.GetOptions{})
	...
	if key.AppSecretName(cr) != "" {
		...
		_, err := r.k8sClient.CoreV1().Secrets(ns).Get(key.AppSecretName(cr), metav1.GetOptions{})
		if apierrors.IsNotFound(err) {
	...
	if key.UserConfigMapName(cr) != "" {
		...
		_, err := r.k8sClient.CoreV1().ConfigMaps(ns).Get(key.UserConfigMapName(cr), metav1.GetOptions{})
		if apierrors.IsNotFound(err) {
	...
	if key.UserSecretName(cr) != "" {
		...
		_, err := r.k8sClient.CoreV1().Secrets(key.UserSecretNamespace(cr)).Get(key.UserSecretName(cr), metav1.GetOptions{})
		if apierrors.IsNotFound(err) {
	...
	if !key.InCluster(cr) {
		...
		_, err := r.k8sClient.CoreV1().Secrets(key.KubecConfigSecretNamespace(cr)).Get(key.KubecConfigSecretName(cr), metav1.GetOptions{})
		if apierrors.IsNotFound(err) {
